...
# Για να λειτουργουν καποια moodules της ttkbootstrap χρειαζεται να γινει overwrite της Image.CUBIC που δεν υπαρχει σε BICUBIC που ειναι το ανανεωμενο version της
...
def appopen():
    ...
    def getdate():
        # Καναμε global την datepicked για να μπορουμε να εχουμε προσβαση σε αυτη για την αποθηκευση των αρχειων
        global datepicked
        # grab date.This is the variable that should be used to open the current date window
        datepicked = dateentry.entry.get()
        strvar.set(value=f"Selected-Date is {datepicked}")
    ...
    # Καλουμε την συναρτηση που θα επιστρεψει τις συνεχωμενες καταγραφες αρχειων για το streaks
    datestreak = creating_files.count_files_with_later_dates()
...
def giannisapp():
    ...
    class App:
        def __init__(self, root):
            ...
            self.w.resizable(
                0, 0
            )  # δεν επιτρέπει στον χρήστη να επεξεργαστεί την γεωμετρια του παραθυρου
            self.w.geometry("715x742")  # Θέτω το μέγεθος του παραθύρου σε 800x775
            self.w.title("Mind at Ease")  # Ορίζω τον τίτλο του παραθύρου
            self.w.configure(bg="#e1c9f8")  # Ορίζω το χρώμα του παραθύρου
    ...
    class Frame1(tk.Frame):
        def __init__(self, parent, app):
            ...
            # Αρχικοποιουμε την λιστα emotions
            self.emotionlist = []
            # Βρισκουμε το αρχειο στο οποιο εχουν ή θα εχουν αποθηκευτουν τα δεδομενα που καταγραφει ο χρηστης
            filename = "_".join(datepicked.split("/")) + ".txt"

            # Καλουμε απο το αρχειο (εαν υπαρχει) τα δεδομενα του χρηστη

            self.file_info_dictionary = creating_files.read_from_files(filename)
            # Αν υπαρχει το file και υπαρχουν πληροφοριες :
            if self.file_info_dictionary is not None:
                # Αποθηκευουμε την λιστα με τα συναισθηματα
                self.emotionlist = self.file_info_dictionary["emotion_list"]
            ...
            for i in range(5):
                ...
                # Αρχικοποιω την μεταβλητη defaultvalue με τιμη κενο string
                defaultvalue = ""

                if i < len(self.emotionlist):
                    # Εαν υπαρχουν αρκετα emotions, τα καλω ενα ενα και τα βαζω στην μεταβλητη defaultvalue για να τα βαλω στο entry
                    defaultvalue = self.emotionlist[i]
                # Δημιουργω μια stringVar με τιμη την defaultvalue
                text = tk.StringVar(value=defaultvalue)
                ...
                self.entry = ttk.Entry(
                    ...
                    font=("Helvetica", 10),
                    bootstyle="danger",
                    # Βαζω την τιμη text στο entry box σαν default τιμη για να μπορει ο χρηστης να την αλλαξει
                    textvariable=text,
                )
                self.entry.grid(row=i + 1, column=1, sticky="news", padx=10, pady=10)
                self.entry.bind("<Key>", self.char_limit)
    ...
    class Frame2(tk.Frame):
        def __init__(self, parent, app):
            ...
            self.app = app
            # Αρχικοποιω την μεταβλητη defaultvalue
            defaultvalue = "    "

            # Εαν υπαρχει το αρχειο με τα δεδομενα και παιρνω το κομματι του text
            if (
                self.app.frame1.file_info_dictionary
                and self.app.frame1.file_info_dictionary["emotion_text"]
            ):
    ...
    class Frame3(tk.Frame):
        ...
        def save_and_quit_window(self):
            self.app.savee()  # Δημιουργώ την συνάρτηση για το κουμπί "Save & Quit", η οποία αποθηκεύει τα στοιχεία του παραθύρου και το κλείνει
            # Καλουμε την συναρτηση που δημιουργει τους φακελους εαν δεν υπαρχουν
            creating_files.create_folder_files()

            # Και δημιουργει αρχειο με τα συναισθηματα και το κειμενο του καθενα με ονομα την ημερομηνια καταγραφης
            creating_files.save_file(datepicked, self.app.entrysave, self.app.textvar)
            self.master.destroy()
...
from pathlib import (
    Path,
)  # Κάνω import την Path απτην pathlib για το path του background
...
class WelcomePage:  # Δημιουργία της κλάσης του παραθύρου
    def __init__(self, root_welcomepage):
        ...
        # Βρισκουμε το αμεσα απο πανω φακελο που βρισκεται το αρχειο main
        folder = Path(__file__).parent
        # Βρισκουμε το path της εικονας που θελουμε να βαλουμε στο background relative του φακελου που βρισκεται το αρχειο main
        image_path = Path(folder, "bg.png")
        self.bg = tk.PhotoImage(file=image_path)
        # Λιστα με τυχαια quotes
        self.randomquotes = [
